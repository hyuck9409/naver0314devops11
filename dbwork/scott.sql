...
--job을 조회하는데 같은 직업은 한번만 나오게, 오름차순으로 출력
select DISTINCT job from emp order by job asc;
select DISTINCT job from emp order by 1;--컬럼번호는 1부터, 위와 같다
...
--컬럼제목을 변경하고싶을 때 
SELECT ENAME "사원명",SAL "급 여" FROM EMP; --제목변경시 공백이 포함되어있을경우 반드시 ""로 묶어준다

--여러칼럼을 합해서 하나의 칼럼으로 출력할경우
...
-- 함수중에 NVL(컬럼명,NULL일경우 초기값),MYSQL은 ifnull 이다
SELECT NVL(COMM,0),NVL(COMM,0)+1000 FROM EMP; --NULL일경우 0으로 출력 또는 계산
...
SELECT MAX(SAL) FROM EMP; -- 최고연봉은?
SELECT MIN(SAL) FROM EMP; -- 최소연봉은?
SELECT SUM(SAL) FROM EMP; -- SAL의 합계
SELECT AVG(SAL) FROM EMP; -- SAL의 평균 
--AVG는 소숫점이하가 많이 나오는 경우가 많아서 ROUNG,CELL,FLOOR 등과 같이 많이 사용
--평균은 소숫점이하 2자리까지만 나오게하고싶다?
SELECT ROUND(AVG(SAL),2) FROM EMP; --2073.21 (반올림함)
SELECT CELL(AVG(SAL)) FROM EMP; -- 무조건 올림 2074
...
SELECT JOB 직업,COUNT(*) 인원,MAX(SAL) 최고연봉,MIN(SAL) 최저연봉,CEIL(AVG(SAL)) 평균연봉 FROM EMP GROUP BY JOB;

-- HAVING 은 GROUP 절에서 그룹함수를 이용해서 조건을 나타낼경우

-- 직업별 인원수와 최고연봉은 구하는데 직업별 인원수가 3명 이상인 경우만 조회를 하시오
...
SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD AM HH:MI') FROM DUAL; -- AM 또는 PM 시간상관없이 사용 가능
SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD HH24:MI') FROM DUAL;

-- EMP 에서 HIREDATE(날짜타입)을 이용해서 출력해보자
