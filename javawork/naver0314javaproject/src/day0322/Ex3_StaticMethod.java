...
class MyShop
{
	...
	private int su,dan;
	private static String message;//공유되는 값 static이 붙어서!
	...
	//상품명,수량,단가를 한번에 변경하고 싶은경우
	public void setData(String sangpum,int su,int dan)
	{
		//메소드안에서 같은 멤버 메소드는 this 로 호출이 가능
	...
	public void setData(String sangpum,int su,int dan,String message)
	{
		...
		this.setSu(su);
		this.setDan(dan);
		//인스턴스메소드에서 static 메소드 호출이 가능할까요?ok 가능
		//반대로 static 메소드엥서 인스턴스 멤버메소드를 호출할 수 있을까요? no 안됨(static은 this가 없어서)
		//static 메소드에서 같은 static 메소드는 호출이 가능할까요?ok 가능
...
public class Ex3_StaticMethod {
	...
	public static void main(String[] args) {
		...
		//쉐어하우스 주방이 공용이다? 주방이 static
		//각 방마다 화장실이 있다? 화장실이 인스턴스
